<template>
  ...
  <el-form ref="AccountFrom" :model="account" :rules="rules" label-position="top" label-width="80px"
    ...
    <h4 style="margin-left: 70px;">个人资料完善</h4>
    ...
    <el-form-item>
      <el-button type="primary" @click="submitForm">完善资料</el-button>
  ...
  export default {
    data() {
      return {
        ...
        rules: {
          nickName: [
            {required: true, message: '请输入昵称称', trigger: 'blur'},
          ...
          phone: [
            {required: false, message: '请输入电话', trigger: 'blur'},
            {
              validator: function (rule, value, callback) {
                //.log(value.length+":"+(value=='') +":"+(/^1[34578]\d{9}|\s+$/.test(value)));
    ...
    methods: {
      submitForm() {
        ...
        this.$refs.AccountFrom.validate((valid) => {
          if (valid) {
            this.loading = true;
            let params = Object.assign({perfectKey: this.key}, this.account);
            console.log(JSON.stringify(params));
            API.perfect(params).then(function (result) {
              that.loading = false;
              if (result.errorCode == '0') {
                that.resetForm('AccountFrom');
                that.$message({showClose: true, message: result.msg || '资料上传成功', type: 'success', duration: 2000});
              } else {
                that.$message.error({showClose: true, message: result.msg || '资料上传失败', duration: 2000});
              }
            }, function (err) {
              that.loading = false;
